...
scale = 1
traj = np.load(r"C:\Users\mohod\Development\OtSim_analysis\data\D3\data\LCP_0.02_6983609.npy")
traj =  traj[5000:,:]
...
while (count < len(traj[:,0])):
    ...
    qu = R.from_quat([traj[count,3],traj[count,4],traj[count,5],traj[count,6]])
    rotMat = qu.as_matrix()
    aor = qu.as_rotvec(degrees=False)
